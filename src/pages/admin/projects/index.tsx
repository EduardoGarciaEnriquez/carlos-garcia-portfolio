import { useEffect, useState } from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { useNavigate } from 'react-router-dom'

import Modal from '../../../components/admin/modal'
import SearchBar from '../../../components/admin/searchbar'
import Table, { ColumnType } from '../../../components/admin/table'
import Button from '../../../components/common/button'
import NoResults from '../../../components/common/noResults'
import Pagination from '../../../components/common/pagination'
import DeleteIcon from '../../../components/icons/delete'
import EditIcon from '../../../components/icons/edit'
import Loader from '../../../components/icons/loader'
import NoImageIcon from '../../../components/icons/noImage'
import {
  deleteProject,
  fetchProjectsByPage,
} from '../../../store/slices/projects.slice'
import { AppDispatch, IRootState } from '../../../store/store'
import { IProject } from '../../../types'

const ProjectsPage = () => {
  const [page, setPage] = useState<number>(1)
  const [search, setSearch] = useState<string | null>(null)
  const [modal, setModal] = useState({ isOpen: false, text: '', id: undefined })

  const {
    projects,
    loadingFetchProjects: loading,
    errorFetchProjects: error,
    totalPages: total,
  } = useSelector((state: IRootState) => state.project)
  const dispatch = useDispatch<AppDispatch>()
  const navigate = useNavigate()

  useEffect(() => {
    dispatch(fetchProjectsByPage({ page, search }))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, search])

  const closeModal = () => {
    setModal({ text: '', isOpen: false, id: undefined })
  }

  const handleOnDelete = () => {
    if (modal?.id) dispatch(deleteProject(modal.id))
    closeModal()
  }

  const PageHeader = () => {
    return (
      <>
        <div className="flex items-center justify-between mb-4 gap-4">
          <h2 className="text-2xl dark:text-gray-400 text-gray-600">
            Projects
          </h2>
          <div className="md:block hidden w-full">
            <SearchBar
              onChangeInput={(value) => {
                setSearch(value)
              }}
              inputValue={search || ''}
            />
          </div>
          <Button
            size="lg"
            variant="filled"
            color="primary"
            onClick={() => {
              navigate('create-new')
            }}
          >
            New
          </Button>
        </div>
        <div className="md:hidden block">
          <SearchBar
            onChangeInput={(value) => {
              setSearch(value)
            }}
            inputValue={search || ''}
          />
        </div>
      </>
    )
  }

  const columns: ColumnType<IProject>[] = [
    {
      title: 'Cover',
      dataIndex: 'cover',
      key: 'cover',
      render: (props) => {
        const { cover, name } = props
        if (!cover) {
          return <NoImageIcon className="h-16 w-auto mx-auto text-gray-400" />
        }
        return (
          <img
            className="h-16 w-auto rounded-md mx-auto"
            src={cover}
            alt={`${name} cover`}
          />
        )
      },
    },
    {
      title: 'Project',
      dataIndex: 'name',
      key: 'name',
      render: (props) => {
        const { name } = props
        return <span className="text-base font-semibold">{name}</span>
      },
    },
    {
      title: 'Actions',
      dataIndex: 'actions',
      key: 'actions',
      render: (props) => {
        const { id, name } = props
        return (
          <div className="flex items-center justify-center gap-2">
            <Button
              size="xs"
              variant="filled"
              color="primary"
              onClick={() => {
                navigate(`${id}`)
              }}
              icon={<EditIcon />}
            />
            <Button
              size="xs"
              variant="outlined"
              color="danger"
              onClick={() => {
                setModal({
                  isOpen: true,
                  text: `Delete project '${name}'?`,
                  id,
                })
              }}
              icon={<DeleteIcon />}
            />
          </div>
        )
      },
    },
  ]

  if (loading)
    return (
      <div className="flex justify-center items-center min-h-[calc(100vh-8rem)]">
        <Loader className="h-24 w-24" />
      </div>
    )

  return (
    <>
      <PageHeader />
      {projects.length !== 0 && !error ? (
        <>
          <Table columns={columns} dataSource={projects ?? []} />
          <Pagination
            total={total}
            page={page}
            next={() => setPage(page + 1)}
            prev={() => setPage(page - 1)}
          />
          <Modal
            open={modal.isOpen}
            onClose={closeModal}
            onAccept={handleOnDelete}
            text={modal.text}
          />
        </>
      ) : (
        <NoResults />
      )}
    </>
  )
}

export default ProjectsPage
